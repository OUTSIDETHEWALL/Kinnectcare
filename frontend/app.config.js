...
module.exports = ({ config }) => ({
  ...
  expo: {
    ...
    android: {
      ...
      versionCode: 51, // Build 51 (Bug #1 hotfix on top of Build 50) — Stale SOS Auto-Resume Trap fixed. 4-layer defense-in-depth: (1) Backend startup migration deletes alerts >30 days old, backfills resolved:false on legacy records, promotes legacy acknowledged SOS to resolved. (2) Frontend auto-resume in _layout.tsx hardened with explicit resolved !== true check, 5-minute staleness gate, session-scoped dismissed-ID set, per-decision diagnostic ring buffer (30 entries). (3) Alert screen (app/alert/[id].tsx) handles 404 on both GET and RESOLVE — clears cached emergency state, marks id dismissed, bounces to Dashboard (never traps user). (4) Dashboard shows red "Unresolved emergency" banner via activeEmergency store for stale-but-unresolved alerts so the user can tap in on their own terms instead of being auto-yanked. All 17 backend tests pass (10 new hotfix + 7 Build 50 regression).
