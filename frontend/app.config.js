...
module.exports = ({ config }) => ({
  ...
  expo: {
    ...
    android: {
      ...
      versionCode: 52, // Build 52 — LOCATION CLARITY & CAREGIVER CONFIDENCE. (1) Bug #2 fix: SOS incident screen tracking-status pill now shares canonical coord+freshness state with MemberMap so they can never disagree; 🔴 "Location unavailable" reserved for the true no-coords case; 🟡 "Last known location" for stale-but-valid coords. (2) Tracking Status UI Refresh: replaced timestamp-centric messaging ("🕒 Updated 3 min ago") with status-centric messaging (🟢 Tracking healthy / 🟡 Location updating… / 🟡 Last known location / 🔴 Location unavailable) across SOS incident screen, member detail screen, and family dashboard cards. Timestamp moved to tiny secondary detail. New shared `<TrackingStatusPill />` component + `computeTrackingStatus()` pure fn at src/tracking/TrackingStatusPill.tsx is the single source of truth every location-status surface consumes. Design language answers "Can I trust that I'm seeing this person's current location?" not "How many minutes ago was this updated?".
