...
module.exports = ({ config }) => ({
  ...
  expo: {
    ...
    android: {
      ...
      versionCode: 57, // Build 57 — PRIVACY INDICATOR & NAV POLISH. Two device-QA bugs from Build #56. (A) P0 Location Sharing UI: turning off correctly stopped uploads (server logs confirmed) but the UI still rendered "🟢 Tracking Healthy" + stale location on BOTH the user's own dashboard card AND family members' devices. Root causes: (1) backend member-doc-clear query required a `family_group_id` match, silently no-op'ing on legacy self-member docs with stale/None family_group_id — DROPPED the family_group_id constraint (user_id is globally unique, so matching on user_id OR owner_id alone is safe). (2) Me tab didn't force a memberStore refetch after toggling, so the caregiver waited a full polling cycle to see the change locally — now calls `fetchAll()` immediately after PUT /me/preferences returns. (3) Dashboard card previously rendered "📍 Location Sharing Off" (server-set clunky placeholder) — now conditionally renders a dedicated "🔒 Location Sharing Off" privacy row IN PLACE OF the pin+location line, and suppresses the TrackingStatusPill entirely (no more "green Healthy" fallback if the sharing_off flag ever arrives late). (4) Also backfill migration at startup ensures every member doc has an explicit `location_sharing_enabled` field defaulting to true. (B) UI POLISH: Me tab icon was rendering as a nearly-invisible "•" dot on device. Root cause: our Icon component is an emoji-based Ionicons drop-in, and `person-circle-outline` had no map entry (fell back to "•"). Fixed both directions — added map entries for person-circle-outline / person-outline / person-circle, AND switched the tab config back to the filled `person` (👤) which matches the visual weight of Family (👨‍👩‍👧) and Alerts (🔔). (C) Charter compliance: still zero new user-facing features per Build #56 charter — every change is either a P0 bug fix or beta-readiness polish.
