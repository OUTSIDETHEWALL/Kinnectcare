...
module.exports = ({ config }) => ({
  ...
  expo: {
    ...
    android: {
      ...
      versionCode: 54, // Build 54 — CAREGIVER CONFIDENCE & ME FOUNDATION. (A) Tracking Confidence Redesign: computeTrackingStatus rewritten from freshness-based to health-based — 🟢 Healthy is the default (no yellow just because someone is stationary); 🟡 Attention reserved for real failure signals (upload/auth failure — escape hatch for Build 55+); 🔴 Offline only when no coords ever OR silent >72 h OR tracking disabled/permissions revoked. Removed "Location updating…" and "Last known location" states — they created caregiver anxiety. (B) Timestamp removal: "Last updated X ago" gone from dashboard cards and member detail primary card; retained on SOS incident screen where freshness is emergency-relevant. (C) Refresh UX: "Requesting fresh location…" spinner row removed from dashboard cards — background refresh is normal operation, no longer telegraphs a health issue. (D) Me tab foundation: new app/(tabs)/me.tsx between Family and Alerts. Sections: Account (read-only), Security (Change PIN), Notifications (Quiet Hours), Privacy (Location Sharing placeholder → Build 55), Support (version + policy links), Advanced (Diagnostics), Sign out. Change Password intentionally omitted — Kinnship is OTP-passwordless. (E) Navigation cleanup: dashboard settings gear now opens /(tabs)/me; family-scoped settings remain under Family tab per "Me = me, Family = everyone" model. Legacy /settings route preserved for now.
