...
module.exports = ({ config }) => ({
  ...
  expo: {
    ...
    android: {
      ...
      versionCode: 56, // Build 56 — RELEASE CANDIDATE / FEATURE FREEZE. Focus: bug fixes, privacy correctness, notification hygiene, UI polish. (A) PRIVACY (P0): Location Sharing OFF now IMMEDIATELY clears the user's own member doc (latitude/longitude/location_name → null, location_sharing_enabled → false) so family devices see "🔒 Location sharing off" on the very next /members poll — no waiting for a fresh coord upload. Server-side upload endpoint (PUT /members/{id}/location) now double-checks the caller's preference and silently no-ops if sharing is OFF, closing the client-side bypass gap Charles caught in Build #55 device QA. New FamilyMember.location_sharing_enabled field mirrored on every doc. (B) TRACKING STATUS: TrackingStatusPill gains a "sharing-off" state that takes precedence over all freshness computation — neutral grey "🔒 Location sharing off" instead of green/yellow/red, so caregivers can't misread a private member as tracking-healthy. Threaded through dashboard cards and member detail. (C) MEMBER DETAIL: sharing-off members render a full "🔒 Location Sharing Off / This family member has chosen not to share their location." banner in place of the pin + map. (D) NOTIFICATIONS (P0): auto-generated proper Kinnship shield+checkmark notification icon (was: generic "K" placeholder — root cause of blank/K-only tray entries). Added client-side defense-in-depth in setNotificationHandler — pushes arriving with empty title AND body are suppressed (never surface as the app-initial fallback). Server-side blank drop already in place from Build #53. (E) ME PAGE POLISH: Account card layout unified — every field now uses label-above-value rhythm (previous build had crowded Email/Role rows). Biometrics section honestly enumerates enrolled types (Fingerprint / Face Unlock / both) instead of always showing "Face Unlock" — matches physical sensors many Android caregivers rely on. (F) NAV: Me tab icon → person-circle-outline (outline-family consistent with Family/Alerts). (G) DIAGNOSTICS: `member_not_found` no longer clutters refresh-trace log — stale client cache hits are treated as expected 404s, not incidents. (H) NO NEW FEATURES per Build #56 charter.
