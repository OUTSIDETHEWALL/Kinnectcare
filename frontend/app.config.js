...
module.exports = ({ config }) => ({
  ...
  expo: {
    ...
    android: {
      ...
      versionCode: 53, // Build 53 — TRACKING CONFIDENCE & NOTIFICATION RELIABILITY (diagnostic-first). No feature creep. (A) Backend blank-notification safety net in expo_push.py: any push that would render visibly with <3 non-whitespace chars is DROPPED and logged to blank_push_drops mongo collection (TTL 24 h) with _source_tag traceability. push_to_family_group auto-injects source_tag = "family:<type>". (B) JWT expiry-claim diagnostic on SDK setAuthToken — every reconfiguration now logs jwt_exp_ms + minutes_until_expiry + already_expired flag; will reveal whether Transistor is holding an expired token. (C) TrackingStatusPill decision ring buffer (30 entries) via trackingPillDiagnostics.ts — every pill render logs {screen, hasCoords, lastSeenIso, ageMs, kind, reason}. (D) Export bg_task_log dedicated button on Diagnostics screen — copies focused execution trace to clipboard. (E) Backend location ingest diagnostic — every successful PUT /api/members/{id}/location writes {writer_user_id, prev_last_seen, new_last_seen, is_self_write, is_owner_write} to location_ingest_log mongo collection (TTL 24 h). Foreground recovery fallback DEFERRED per user directive — observe first, repair second.
