...
// expo-battery intentionally omitted — native module is not in the current
// binary (runtime 1.2.0 was built before this package was added).  Adding
// a top-level import of a missing native module crashes the JS bundle at
// initialization, taking down every screen that imports locationRefresh.
// Re-introduce battery reading in the next native build.
...
export async function refreshLocationIfStale(reason: string): Promise<void> {
  try {
    ...
    try {
      ...
      // battery_level / is_charging omitted until native build includes expo-battery
